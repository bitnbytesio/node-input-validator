...
class Validator {
  ...
  async apply(attr) {
    ...
    for (const key of attr.split('.')) {
      ...
      if (state[key] == undefined && typeof validation.value === 'object') {
        // @ts-ignore
      ...
      //   state = state[key];
      // }
      console.log('loop  :', attr, key, state);
      // /state[key] = validation.value;
    }
    console.log('after:', state, validation.value)
