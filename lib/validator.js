...
// @beta feature: collect inputs as per declared rules (disabled by default)
let shouldCollectDeclaredInputs = false;
...
class Validator {
  ...
  /**
   * @beta
   * globally enable/disable collection of inputs as per declared rules
   * when enabled, validated inputs can be retrieved via data() method
   * @param {boolean} enable
   */
  static collectInputs(enable) {
    shouldCollectDeclaredInputs = enable;
  }
  ...
  async parseNestedAttr(attr) {
    ...
    Object.keys(value).forEach(() => {
      ...
      validations.push(this.apply(absKey));
      index++;
    });
  ...
  async apply(attr) {
    ...
    if (attr.indexOf('*') >= 0) {
      await this.parseNestedAttr(attr);
      return;
    }
    ...
    // @beta feature: collect inputs as per declared rules
    if (shouldCollectDeclaredInputs) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (i === lastIndex) {
          ...
          // Intermediate key exists and is an object - traverse
          state = state[key];
        } else {
          // Intermediate key doesn't exist or is primitive - create object/array
          const nextKey = keys[i + 1];
          // eslint-disable-next-line no-multi-assign
          state = state[key] = /^\d+$/.test(nextKey) ? [] : {};
        }
      }
    }
  }
