...
class Validator {
  ...
  parseRules(validationRules) {
    ...
    attrs.forEach((attr) => {
      ...
      // if array of rules
      if (Array.isArray(strRules)) {
        strRules.forEach((ruleItem) => {
          let args = [];
          let rule = ruleItem;
          if (Array.isArray(ruleItem)) {
            [rule, ...args] = ruleItem;
          }
          attrRule.rules.push({ rule, args });
        });
      ...
      const rulesArray = strRules.toString().split('|');

      // parse array of string rules
      rulesArray.forEach((rule) => {
        // split by colon to get rule name and args
        const [ruleName, argsStr] = rule.split(':');
        // if rule is nullable
 ...
 */
  ...
  getParsedMessage({
    ...
    */
    ...
    if (!message) {
      message = (messages[this.lang].custom && messages[this.lang].custom[`${attr}.${rule}`])
