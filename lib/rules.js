...
class Rules extends Mix {
    ...
    async validateRequiredIf(field, value, args) {
        ...
        for (let start = 0; start < args.length; start += 2) {
            ...
            if (typeof this.validator.inputs[requiredField] !== 'undefined'
    ...
    async validateRequiredWith(field, value, args) {
        ...
        for (var i = 0; i < args.length; ++i) {
            ...
            if (typeof this.validator.inputs[args[i]] !== 'undefined' 
                && this.validator.inputs[args[i]].toString().trim() != '' ) {
                required = true;
                break;
            }
        }
    ...
    async validate(field) {
        ...
        for (let r = 0; r < field.rules.length; r++) {
            ...
            if ( !field.required && (typeof field.value == 'undefined' || field.value.toString().trim() == ''))   {
                continue;
            }   
