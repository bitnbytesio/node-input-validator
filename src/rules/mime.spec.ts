...
describe('rule:mime', () => {
  test('should pass', async () => {
    ...
    expect(await mime(['png', 'jpg'], true).handler({ type: 'image/jpeg' })).toBe(true);
    expect(await mime(['image/png', 'image/jpg'], true).handler({ mime: 'image/png' })).toBe(true);
    expect(await mime(['png', 'jpg']).handler(filePath)).toBe(true);
    expect(await mime(['image/png', 'jpg']).handler({ path: filePath })).toBe(true);
